...
\begin{itemize}
  \item Best-fit global H0 (full Planck posterior, Section 2.4.2): 67.83 km/\allowbreak{}s/\allowbreak{}Mpc
  \item Void enhancement factor: S0 = 1.084 (the algebraic identity S0 = $\alpha$$\sqrt{\,}$$\Omega$\_\allowbreak{}DE, evaluated at KBC-level void depth)
  \item Implied local value: H\_\allowbreak{}local $\approx$ 73.5 km/\allowbreak{}s/\allowbreak{}Mpc, conditional on a deep KBC-like local underdensity; shallower void reconstructions give roughly 70 to 71 km/\allowbreak{}s/\allowbreak{}Mpc
...
$\dagger$ Shift column: a descriptive separation in units of the $\Lambda$CDM posterior width, not a detection significance.

All parameter shifts occur in the direction that reduces the S\textsubscript{8} and Hubble tensions; no parameter exhibits pathological broadening. The dominant effect is a 2.4\% downward shift in $\sigma$\textsubscript{8} (0.8101 to 0.7903; a 3.4$\sigma$ separation in units of the $\Lambda$CDM posterior width, not a detection significance), accompanied by a modest increase in H\textsubscript{0} and n\textsubscript{s}, while geometric parameters ($\theta$\textsubscript{s}, $\omega$\textsubscript{b}) are essentially unchanged. Error bars are comparable in both models, confirming that the additional k\textsubscript{f} parameter does not introduce degeneracies that inflate uncertainties.

\textbf{Interpretation of $\omega$\textsubscript{cdm} in MTDF fits.} In the current class\_\allowbreak{}mtdf implementation, the early-universe matter density parameter $\omega$\textsubscript{cdm} is retained and interpreted as the stress-field energy component that is constructed to be degenerate with cold dark matter at early times (the field's pre-developed phase; see MTDF\_\allowbreak{}01 Section 5.3). MTDF's no-dark-matter claim is that no particle component is required at late times and on galactic scales, where the field's environment-dependent dynamics replace halo phenomenology; it is not a claim that the early-universe energy budget lacks a CDM-like component. A first-principles derivation showing that the developed field relaxes into the late-time phenomenology from this early degenerate phase is an open theoretical task.
...
\begin{itemize}
  \item \textbf{{\small\protect\path{phase5_triangle.png}}}: Joint posterior contours for all cosmological parameters in both models, showing the shift in $\sigma$\textsubscript{8} and H\textsubscript{0}.
  \item \textbf{{\small\protect\path{phase5_kf_posterior.png}}}: Marginalised posterior of k\textsubscript{f} with 68\% and 95\% credible intervals marked.
\end{itemize}

\begin{mdframed}[linecolor=accent,linewidth=0.5pt,backgroundcolor=callout-bg,leftline=true,rightline=true,topline=true,bottomline=true,innerleftmargin=8pt,innerrightmargin=8pt,innertopmargin=6pt,innerbottommargin=6pt]
\textbf{Reproducibility.} Full chain files (\texttt{{\small\protect\path{lcdm_mcmc.1.txt}}}, \texttt{{\small\protect\path{mtdf_mcmc.1.txt}}}), Cobaya YAML configurations, checkpoint files, and the analysis scripts (\texttt{{\small\protect\path{analyze_phase5.py}}}, \texttt{{\small\protect\path{kf_analysis.py}}}) are provided in the repository under \texttt{results/\allowbreak{}phase5/\allowbreak{}}. All numbers in this section are computed from the converged chains with 30\% burn-in applied.
\end{mdframed}
