...
MTDF's structural suppression ensures all Post-Newtonian (PPN) parameters remain safe by factors exceeding $10^{20}$ at scales $< 1$ AU. Wide binary tests are consistent with effectively Newtonian dynamics ($\gamma = 1.10 \pm 0.01$, below the 1.15 threshold; $\gamma$ tracks the contamination fraction (r = 0.99) and falls to 1.06--1.08 under quality cuts). This constitutes a clean external discriminator: MTDF and acceleration-threshold models (e.g. MOND) agree on galaxy-scale phenomenology but differ sharply for wide binaries, where MTDF predicts Newtonian behaviour because the stress field is sourced by galaxy-integrated mass, not by individual stellar systems.
