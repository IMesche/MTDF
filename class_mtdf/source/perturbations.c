...
int perturbations_einstein(
  ...
  double shear_g = 0.;
  double shear_idr = 0.;
  double mu_mtdf = 1.0;  /* MTDF modified gravity coupling */
  ...
  /* MTDF: compute modified gravity coupling μ(a) */
  if (pba->has_mtdf == _TRUE_ && pba->mtdf.has_mtdf_growth == _TRUE_) {
    mu_mtdf = mtdf_mu(a, &(pba->mtdf));
  }
...
int perturbations_derivs(double tau,
  ...
  /* MTDF modified gravity coupling μ(a) */
  double mu_mtdf = 1.0;
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppt->gauge == newtonian) {
        ...
        /* MTDF: CDM feels enhanced gravity via μ(a) */
        dy[pv->index_pt_theta_cdm] = - a_prime_over_a*y[pv->index_pt_theta_cdm] + mu_mtdf * metric_euler; /* cdm velocity */
      }

      /** - ----> synchronous gauge: cdm density only (velocity set to zero by definition of the gauge) */
      /** Note: in synchronous gauge, μ(a) modification is applied via h' equation in perturbations_einstein */
