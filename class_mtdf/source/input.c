...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** 8.c) MTDF (Mesche Tensor Dynamics Framework) parameters */
  /* Read the enable flag */
  class_call(parser_read_string(pfc,"mtdf",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  ...
  if ((flag1 == _TRUE_) && ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL))) {
    ...
    /* MTDF fundamental parameters */
    /* Default values: alpha=1.30, beta_eos=0.573, z_t=0.74 */
    pba->mtdf.alpha = 1.30;
    pba->mtdf.beta_eos = 0.573;
    pba->mtdf.z_t = 0.74;

    class_read_double("mtdf_alpha",pba->mtdf.alpha);
    class_read_double("mtdf_beta_eos",pba->mtdf.beta_eos);
    class_read_double("mtdf_z_t",pba->mtdf.z_t);

    /* k_f: EFE amplitude scaling factor for MCMC (default: 1.0) */
    pba->mtdf.k_f = 1.0;
    class_read_double("mtdf_k_f",pba->mtdf.k_f);

    /* EFE profile shape parameters (for diagnostic scans) */
    pba->mtdf.z_peak = 3400.0;
    pba->mtdf.sigma_z = 0.5;
    class_read_double("mtdf_z_peak",pba->mtdf.z_peak);
    class_read_double("mtdf_sigma_z",pba->mtdf.sigma_z);

    /* EFE and growth modification flags (default: both enabled) */
    pba->mtdf.has_mtdf_efe = _TRUE_;
    pba->mtdf.has_mtdf_growth = _TRUE_;

    class_call(parser_read_string(pfc,"mtdf_efe",&string1,&flag1,errmsg),
               errmsg,
               errmsg);
    if ((flag1 == _TRUE_) && ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL))) {
      pba->mtdf.has_mtdf_efe = _FALSE_;
    }

    class_call(parser_read_string(pfc,"mtdf_growth",&string1,&flag1,errmsg),
               errmsg,
               errmsg);
    if ((flag1 == _TRUE_) && ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL))) {
      pba->mtdf.has_mtdf_growth = _FALSE_;
    }

    /* Initialize derived MTDF parameters */
    class_call(mtdf_init(&(pba->mtdf), pba->Omega0_lambda),
               pba->error_message,
               errmsg);

    if (pba->background_verbose > 0) {
      mtdf_print_parameters(&(pba->mtdf));
    }
  }
  else {
    pba->has_mtdf = _FALSE_;
  }
