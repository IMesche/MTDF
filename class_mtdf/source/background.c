...
int background_functions(
  ...
  /* MTDF early field energy (EFE)
   *
   * The EFE adds energy density as a FRACTION of the total.
   * If Omega_efe is the fractional contribution, then:
   *   rho_tot_new = rho_tot_old + rho_efe
   *   Omega_efe = rho_efe / rho_tot_new
   * Solving: rho_efe = Omega_efe * rho_tot_old / (1 - Omega_efe)
   */
  if (pba->has_mtdf == _TRUE_) {
    double z = 1./a - 1.;
    double omega_efe = mtdf_omega_efe(z, &(pba->mtdf));
    double w_efe = mtdf_w_efe(z, &(pba->mtdf));

    /* Compute EFE density as fraction of total */
    double rho_efe;
    if (omega_efe > 0 && omega_efe < 1.0) {
      rho_efe = omega_efe * rho_tot / (1.0 - omega_efe);
    } else {
      rho_efe = 0.0;
    }

    pvecback[pba->index_bg_rho_mtdf_efe] = rho_efe;
    pvecback[pba->index_bg_w_mtdf_efe] = w_efe;

    rho_tot += rho_efe;
    p_tot += w_efe * rho_efe;

    /* dp/dloga for EFE: approximate as stiff fluid contribution */
    dp_dloga += -(3.0 * (1.0 + w_efe) * w_efe) * rho_efe;

    /* EFE contribution to relativistic density (w > 1/3 means faster than radiation) */
    if (w_efe > 0.0) {
      /* Fractional assignment based on effective w */
      double frac_rel = (3.0 * w_efe) / (1.0 + 3.0 * w_efe);
      rho_r += frac_rel * rho_efe;
    }
  }
...
int background_indices(
  ...
  /* Note: has_mtdf is set in input.c, not here - do not reset it */
  ...
  /* - index for MTDF early field energy */
  class_define_index(pba->index_bg_rho_mtdf_efe,pba->has_mtdf,index_bg,1);
  class_define_index(pba->index_bg_w_mtdf_efe,pba->has_mtdf,index_bg,1);
