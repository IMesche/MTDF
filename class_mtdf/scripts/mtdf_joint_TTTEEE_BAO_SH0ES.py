#!/usr/bin/env python3
"""
MTDF Joint MCMC: Planck TTTEEE + DESI BAO + SH0ES H0 Prior

Full constraint on MTDF including SH0ES local H0 measurement.
...
def run_mcmc(z_bao, obs_bao, types_bao, cov_bao,
    ...
    # Start H0 slightly higher since SH0ES pulls toward 73
    p0_center = np.array([70.0, 0.02237, 0.1200, 0.9649, 3.044, 0.0544, 0.2])
...
def main():
    ...
    np.save(f"{output_dir}/mtdf_chain_TTTEEE_BAO_SH0ES.npy", samples)
    np.savez(f"{output_dir}/mtdf_best_TTTEEE_BAO_SH0ES.npz",
    ...
    with open(f"{output_dir}/mtdf_joint_TTTEEE_BAO_SH0ES_results.txt", 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("MTDF Joint MCMC Results (Planck TTTEEE + DESI BAO + SH0ES)\n")
        f.write("=" * 70 + "\n\n")
    ...
    print(f"\nResults saved to {output_dir}/mtdf_*_TTTEEE_BAO_SH0ES.*")
