#!/usr/bin/env python3
"""
MTDF Joint MCMC: Planck TTTEEE + DESI BAO + SH0ES (with Void Enhancement)

This script properly models the MTDF two-component H0 prediction:

1. EARLY UNIVERSE (Acoustic Compression):
   - The stress field "rings" at matter-radiation equality
   - Shrinks sound horizon by ~0.74% (f_kick ~ 0.33%)
   - Gives H0_global ~ 67.4 + 0.1 km/s/Mpc

2. LATE UNIVERSE (Void Stress Depletion):
   - We live in a local cosmic void
   - Voids have lower field stress than background
   - This "stress vacuum" enhances LOCAL expansion rate
   - H0_local = H0_global + delta_H0_void(k_f)

The SH0ES measurement is EXPECTED to differ from Planck in MTDF!
- Planck sees H0_global (no local void effect)
- SH0ES sees H0_local (enhanced by void depletion)
"""
...
# SH0ES prior (Riess et al. 2022) - this is a LOCAL measurement
SHOES_H0 = 73.04
SHOES_SIGMA = 1.04
...
# MTDF Void Enhancement Model
# ============================
# The local H0 enhancement from void stress depletion scales with k_f:
#   delta_H0_void = VOID_ENHANCEMENT_COEFF * k_f
#
# Calibration: At k_f ~ 0.1, we expect ~5.7 km/s/Mpc enhancement
#   => VOID_ENHANCEMENT_COEFF = 5.7 / 0.1 = 57 km/s/Mpc per unit k_f
#
# This gives: H0_local = H0_global + 57 * k_f
VOID_ENHANCEMENT_COEFF = 57.0  # km/s/Mpc per unit k_f

# Parameter priors
PRIORS = {
    'H0': (60.0, 75.0),  # H0_global (what Planck constrains)
    'omega_b': (0.019, 0.025),
    'omega_cdm': (0.10, 0.14),
    'n_s': (0.9, 1.05),
    'logA': (2.9, 3.2),
    'tau_reio': (0.04, 0.12),
    'mtdf_k_f': (0.0, 0.3),  # Allow wider range for void enhancement
}
...
# =============================================================================
# MTDF Void Enhancement
# =============================================================================

def compute_H0_local(H0_global, k_f):
    """Compute LOCAL H0 from global H0 + void stress depletion.

    In MTDF, we live in a local cosmic void where stress is depleted.
    This enhances the local expansion rate measured by SH0ES.

    H0_local = H0_global + VOID_ENHANCEMENT_COEFF * k_f

    At k_f ~ 0.1: H0_local ≈ H0_global + 5.7 km/s/Mpc
    """
    delta_H0_void = VOID_ENHANCEMENT_COEFF * k_f
    return H0_global + delta_H0_void
...
def compute_model(params_dict):
    """Compute Cls and distances for given parameters."""
    cosmo = classy.Class()
    ...
    params = {
        ...
        'H0': params_dict['H0'],  # This is H0_global
...
def log_likelihood_shoes(H0_local):
    """Compute SH0ES H0 prior log-likelihood using LOCAL H0.

    CRITICAL: SH0ES measures the LOCAL expansion rate, not the global one.
    In MTDF, H0_local = H0_global + void_enhancement(k_f).
    """
    chi2 = ((H0_local - SHOES_H0) / SHOES_SIGMA) ** 2
    return -0.5 * chi2
...
def log_probability(theta, z_bao, obs_bao, types_bao, cov_bao):
    """Total log-probability (TTTEEE + low-l + BAO + SH0ES with void enhancement)."""
    lp = log_prior(theta)
    ...
    try:
        ...
        # High-l TTTEEE (uses H0_global via CMB physics)
        ...
        # BAO (uses H0_global for distance ratios)
        ll += log_likelihood_bao(z_bao, obs_bao, types_bao, cov_bao,
                                  bg, r_d, params_dict['H0'])

        # SH0ES H0 prior - USES H0_LOCAL (with void enhancement)
        H0_global = params_dict['H0']
        k_f = params_dict['mtdf_k_f']
        H0_local = compute_H0_local(H0_global, k_f)
        ll += log_likelihood_shoes(H0_local)
...
def run_mcmc(z_bao, obs_bao, types_bao, cov_bao,
    ...
    # Initialize walkers
    # H0_global ~ 67.4, k_f ~ 0.1 should give H0_local ~ 73.1
    p0_center = np.array([67.4, 0.02237, 0.1200, 0.9649, 3.044, 0.0544, 0.10])
    p0 = p0_center + 0.01 * p0_center * np.random.randn(nwalkers, ndim)
    p0[:, 6] = np.abs(p0[:, 6])  # Ensure k_f stays positive
    ...
    chi2_bao = -2 * log_likelihood_bao(z_bao, obs_bao, types_bao, cov_bao,
                                        bg, r_d, H0_global)
    chi2_shoes = -2 * log_likelihood_shoes(H0_local)

    return (samples, log_probs, best_params, r_d, r_s,
            chi2_ttteee, chi2_lowl_tt, chi2_lowl_ee, chi2_bao, chi2_shoes,
            H0_global, H0_local)
...
def main():
    print("=" * 70)
    print("MTDF Joint MCMC: Planck TTTEEE + DESI BAO + SH0ES")
    print("            (with Void Stress Depletion Enhancement)")
    print("=" * 70)

    print(f"\n*** MTDF Two-Component H0 Model ***")
    print(f"  H0_local = H0_global + {VOID_ENHANCEMENT_COEFF} * k_f")
    print(f"  At k_f = 0.10: H0_local = H0_global + 5.7 km/s/Mpc")
    print(f"  This explains why Planck and SH0ES see different H0!")

    print(f"\nLikelihoods enabled:")
    print(f"  Planck high-l TTTEEE: True (constrains H0_global)")
    print(f"  Planck low-l TT:      True")
    print(f"  Planck low-l EE:      True")
    print(f"  DESI BAO:             True (constrains H0_global)")
    print(f"  SH0ES H0:             True (constrains H0_LOCAL = {SHOES_H0} +/- {SHOES_SIGMA})")
    ...
    # Initialize Planck likelihoods
    init_planck_likelihoods()

    # Run MCMC
    results = run_mcmc(z_bao, obs_bao, types_bao, cov_bao,
                       nwalkers=16, nburn=50, nprod=100)

    (samples, log_probs, best, r_d, r_s,
     chi2_ttteee, chi2_lowl_tt, chi2_lowl_ee, chi2_bao, chi2_shoes,
     H0_global_best, H0_local_best) = results
    ...
    print(f"\n*** H0 Decomposition ***")
    print(f"  H0_global (Planck sees): {H0_global_best:.2f} km/s/Mpc")
    delta_H0 = H0_local_best - H0_global_best
    print(f"  Void enhancement:        +{delta_H0:.2f} km/s/Mpc")
    print(f"  H0_local (SH0ES sees):   {H0_local_best:.2f} km/s/Mpc")
    ...
    print(f"  SH0ES H0:             {chi2_shoes:.2f} (1 data point, using H0_local)")
    print(f"  Total:                {chi2_total:.2f}")

    # Tension with SH0ES (should be MUCH reduced!)
    tension_sigma = abs(H0_local_best - SHOES_H0) / SHOES_SIGMA
    print(f"\n  H0_local = {H0_local_best:.2f} km/s/Mpc")
    print(f"  Tension with SH0ES: {tension_sigma:.1f}σ")
    ...
    # Compute H0_local distribution
    H0_global_samples = samples[:, 0]
    k_f_samples = samples[:, 6]
    H0_local_samples = H0_global_samples + VOID_ENHANCEMENT_COEFF * k_f_samples

    H0_local_percentiles = np.percentile(H0_local_samples, [16, 50, 84])
    H0_local_med = H0_local_percentiles[1]
    H0_local_lower = H0_local_med - H0_local_percentiles[0]
    H0_local_upper = H0_local_percentiles[2] - H0_local_med
    print(f"\n  {'H0_local':12s} = {H0_local_med:.4f} +{H0_local_upper:.4f} -{H0_local_lower:.4f}")

    # Save results
    output_dir = str(Path(__file__).parent.parent / 'output')

    np.save(f"{output_dir}/mtdf_chain_TTTEEE_BAO_SH0ES_void.npy", samples)
    np.savez(f"{output_dir}/mtdf_best_TTTEEE_BAO_SH0ES_void.npz",
             ...
             r_d=r_d, r_s=r_s,
             H0_global=H0_global_best, H0_local=H0_local_best,
             void_enhancement_coeff=VOID_ENHANCEMENT_COEFF,
             shoes_h0=SHOES_H0, shoes_sigma=SHOES_SIGMA)

    # Save summary text file
    with open(f"{output_dir}/mtdf_joint_TTTEEE_BAO_SH0ES_void_results.txt", 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("MTDF Joint MCMC Results (with Void Stress Depletion)\n")
        f.write("=" * 70 + "\n\n")

        f.write("MTDF Two-Component H0 Model:\n")
        f.write(f"  H0_local = H0_global + {VOID_ENHANCEMENT_COEFF} * k_f\n")
        f.write(f"  Planck constrains H0_global (CMB physics)\n")
        f.write(f"  SH0ES constrains H0_local (local void enhancement)\n\n")

        f.write("MTDF fixed parameters:\n")
        f.write(f"  alpha    = {MTDF_ALPHA}\n")
        f.write(f"  beta_eos = {MTDF_BETA_EOS}\n")
        f.write(f"  z_t      = {MTDF_Z_T}\n\n")

        f.write(f"SH0ES prior: H0_local = {SHOES_H0} +/- {SHOES_SIGMA} km/s/Mpc\n\n")
        ...
        f.write("H0 Decomposition:\n")
        f.write(f"  H0_global (Planck sees): {H0_global_best:.2f} km/s/Mpc\n")
        f.write(f"  Void enhancement:        +{delta_H0:.2f} km/s/Mpc\n")
        f.write(f"  H0_local (SH0ES sees):   {H0_local_best:.2f} km/s/Mpc\n\n")
        ...
        f.write(f"  SH0ES H0:             {chi2_shoes:.2f} (1 data point, H0_local)\n")
    ...
    print(f"\nResults saved to {output_dir}/mtdf_*_TTTEEE_BAO_SH0ES_void.*")
