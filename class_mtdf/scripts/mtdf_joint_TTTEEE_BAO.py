#!/usr/bin/env python3
"""
MTDF Joint MCMC: Planck TTTEEE + DESI BAO (no SH0ES)

Clean CMB+BAO-only constraint on MTDF with full polarization.
...
def compute_model(params_dict):
    ...
    params = {
        ...
        'mtdf': 'yes',
        'mtdf_efe': 'yes',
        'mtdf_growth': 'yes',  # Enable μ(a) for self-consistent growth
...
def log_likelihood_ttteee(cl_tt, cl_ee, cl_te, cl_bb):
    ...
    # lmax = [lmax_TT, lmax_EE, lmax_BB, lmax_TE, ...]
    lmax_tt = lmax[0]
    lmax_ee = lmax[1]
    lmax_bb = lmax[2]
    lmax_te = lmax[3]

    # Build input vector: TT, EE, BB, TE, A_planck
...
def log_probability(theta, z_bao, obs_bao, types_bao, cov_bao):
    """Total log-probability (TTTEEE + low-l + BAO, no SH0ES)."""
    lp = log_prior(theta)
...
def run_mcmc(z_bao, obs_bao, types_bao, cov_bao,
    ...
    # Initialize walkers near Planck best-fit with k_f near 0
    p0_center = np.array([67.5, 0.02237, 0.1200, 0.9649, 3.044, 0.0544, 0.1])
    p0 = p0_center + 0.01 * p0_center * np.random.randn(nwalkers, ndim)
    # Ensure k_f stays positive
    p0[:, 6] = np.abs(p0[:, 6])
    ...
    return samples, log_probs, best_params, r_d, r_s, chi2_ttteee, chi2_lowl_tt, chi2_lowl_ee, chi2_bao
...
def main():
    print("=" * 70)
    print("MTDF Joint MCMC: Planck TTTEEE + DESI BAO (no SH0ES)")
    print("=" * 70)
    ...
    print(f"  SH0ES H0:             False (excluded)")
    ...
    # Run MCMC
    samples, log_probs, best, r_d, r_s, chi2_ttteee, chi2_lowl_tt, chi2_lowl_ee, chi2_bao = run_mcmc(
    ...
    chi2_total = chi2_planck_total + chi2_bao
    ...
    np.save(f"{output_dir}/mtdf_chain_TTTEEE_BAO_mu.npy", samples)
    np.savez(f"{output_dir}/mtdf_best_TTTEEE_BAO_mu.npz",
    ...
    with open(f"{output_dir}/mtdf_joint_TTTEEE_BAO_mu_results.txt", 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("MTDF Joint MCMC Results (Planck TTTEEE + DESI BAO, with μ(a))\n")
        f.write("=" * 70 + "\n\n")
    ...
    print(f"\nResults saved to {output_dir}/mtdf_*_TTTEEE_BAO_mu.*")
