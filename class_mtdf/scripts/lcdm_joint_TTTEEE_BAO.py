#!/usr/bin/env python3
"""
LCDM Joint MCMC: Planck TTTEEE + DESI BAO (no SH0ES)

Clean CMB+BAO-only constraint on standard LCDM for comparison with MTDF.
...
def log_probability(theta, z_bao, obs_bao, types_bao, cov_bao):
    """Total log-probability (TTTEEE + low-l + BAO)."""
    lp = log_prior(theta)
...
def main():
    print("=" * 70)
    print("LCDM Joint MCMC: Planck TTTEEE + DESI BAO (no SH0ES)")
    print("=" * 70)
    ...
    np.save(f"{output_dir}/lcdm_chain_TTTEEE_BAO.npy", samples)
    np.savez(f"{output_dir}/lcdm_best_TTTEEE_BAO.npz",
    ...
    with open(f"{output_dir}/lcdm_joint_TTTEEE_BAO_results.txt", 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("LCDM Joint MCMC Results (Planck TTTEEE + DESI BAO, no SH0ES)\n")
        f.write("=" * 70 + "\n\n")
    ...
    print(f"\nResults saved to {output_dir}/lcdm_*_TTTEEE_BAO.*")
