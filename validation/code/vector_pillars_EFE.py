"""
vector_pillars_EFE.py
Vector pillar infrastructure for MTDF validation with Early Field Energy (EFE).
...
- MTDF model prediction functions with EFE corrections
...
# =============================================================================
# WORKBOOK COEFFICIENT LOADER
# =============================================================================

def _get_workbook_path():
    """Get path to the MTDF workbook."""
    # Navigate from code/ to data/
    code_dir = Path(__file__).parent
    return code_dir.parent / "data" / "DB_Workbook_STRICT_V18.xlsx"


@lru_cache(maxsize=1)
def _load_coefficients():
    """
    Load coefficients from Params_Coefficients sheet.

    Returns dict mapping input_token -> value_si
    """
    wb_path = _get_workbook_path()
    ...
    for _, row in df.iterrows():
        token = row.get('input_token')
        value = row.get('value_si')
        if pd.notna(token) and pd.notna(value):
            # Skip non-numeric values like "variable"
            try:
                coeffs[token] = float(value)
...
def get_coefficient(token, default=None):
    """
    Get a coefficient value from the workbook by its input_token.

    Args:
        token: The input_token identifier (e.g., 'K_RS_EFE')
    ...
    try:
        ...
        return coeffs.get(token, default)
    except Exception:
        return default


# Sound horizon ratio: r_s(z*) / r_d - loaded from Params_Coefficients
# Calibrated once by matching the analytic r_s(z*)/r_d to Planck 2018 ΛCDM baseline.
# This is a fixed compression constant (not a free MTDF parameter).
RS_ZSTAR_RATIO = get_coefficient('RS_ZSTAR_RATIO', default=0.9819)

# =============================================================================
# MTDF EARLY FIELD ENERGY CONSTANTS (Updated normalization, v17.1)
# =============================================================================
# These constants are derived from MTDF field dynamics using first-principles.
# They enable the EFE correction to the recombination sound horizon.
#
# First-principles derivation (see MTDF_C_MTDF_Derivation_v2.md):
#   f_kick = λ_MTDF / 24 ≈ 0.0033 (0.33%)
#
# The factor 24 comes from γ_int = 1/12:
...
#
# For backward compatibility, we can express this as:
#   f_kick = C_MTDF_EFE * Ω_Λ * λ_MTDF
#   with C_MTDF_EFE = 1 / (24 * Ω_Λ) ≈ 0.06
#
# The old value C_MTDF_EFE = 0.61 was back-calculated from a desired H₀ and
# is now replaced by this first-principles derivation.

# K_RS_EFE: Mapping constant relating f_kick to fractional sound horizon shift.
# Defined as (Δr_s / r_s) / f_kick at the MTDF calibration point.
# Derived from dedicated r_s(f_kick) integral scans. Stable to ~10% across
# reasonable parameter variations. NOW READ FROM WORKBOOK.
K_RS_EFE = get_coefficient('K_RS_EFE', default=-0.215)
...
def mtdf_cmb_distance_vector_efe(params, return_diagnostics=False):
    """
    MTDF CMB distance prior with early field energy (EFE) correction.

    This is the Option C implementation that applies MTDF's predicted
    early field energy to the recombination sound horizon. It keeps the
    same baseline physics as mtdf_cmb_distance_vector, then applies:

        r_s_mtdf = r_s_std * (1 + K_RS_EFE * f_kick)

    where:
        lambda_mtdf = (1 - beta_eos)^2 / (1 + alpha)
        f_kick      = lambda_mtdf / 24  (first-principles derivation)

    The correction is derived from MTDF field dynamics and is NOT tuned
    to CMB data. At the current calibration point (α ≈ 1.3, β_eos ≈ 0.573):
        - lambda_mtdf ≈ 0.079
        - f_kick ≈ 0.33% (0.0033)
        - Δr_s/r_s ≈ -0.07%

    The EFE amplitude is fixed by theory (k_f = 1.0 in CLASS notation).
    A separate CLASS + Planck TTTEEE + DESI BAO analysis confirms k_f ≈ 1.0 ± 0.1.

    Args:
        params: dict with keys:
            - H0, omegab_h2, omegam_h2 (same as baseline)
            - alpha: MTDF stress-matter coupling
            - beta_eos: MTDF EoS transition parameter
        return_diagnostics: if True, return (prediction, diagnostics) tuple

    Returns:
        Array [R, lA, omegab_h2] and optionally diagnostics dict with:
            - f_kick, lambda_mtdf, r_s_std, r_s_mtdf, delta_rs_over_rs
            - lA_std, lA_mtdf, R, z_star, D_M_star, chi2 components
    """
    # === BASELINE PARAMETERS (same as mtdf_cmb_distance_vector) ===
    H0 = params.get('H0', 70.0)
    h = params.get('h', H0 / 100.0)
    ...
    # Omega_L from flatness closure (consistent with baseline)
    Omega_L = 1.0 - Omega_m - Omega_r

    # Hubble distance D_H = c/H0 in Mpc
    D_H = C_LIGHT / H0

    # === COMOVING DISTANCE (identical to baseline) ===
    n_steps = 5000
    z_arr = np.linspace(0, z_star, n_steps)
    E_z = np.sqrt(Omega_r * (1+z_arr)**4 + Omega_m * (1+z_arr)**3 + Omega_L)
    integrand = 1.0 / E_z
    ...
    # === SOUND HORIZON (baseline from Aubourg) ===
    r_d, r_s_std = compute_sound_horizon_from_densities(params)

    # === MTDF EARLY FIELD ENERGY CORRECTION ===
    # Extract MTDF parameters (with defaults matching workbook)
    alpha = params.get('alpha', 1.3)
    beta_eos = params.get('beta_eos', 0.573)

    # MTDF combination: λ_MTDF = (1 - β_eos)² / (1 + α)
    lambda_mtdf = ((1.0 - beta_eos) ** 2) / (1.0 + alpha)

    # Early field energy fraction: f_kick = λ_MTDF / 24 (first-principles)
    # This replaces the old back-calculated formula: f_kick = C_MTDF_EFE * Ω_Λ * λ_MTDF
    f_kick = lambda_mtdf / 24.0

    # Safety clamp (should never trigger for physical parameters)
    if f_kick < 0.0:
        f_kick = 0.0

    # Apply correction: r_s_mtdf = r_s_std * (1 + K_RS_EFE * f_kick)
    # At calibration point: K_RS_EFE * f_kick ≈ -0.215 * 0.0033 ≈ -0.0007 → Δr_s/r_s ≈ -0.07%
    r_s_mtdf = r_s_std * (1.0 + K_RS_EFE * f_kick)

    # Fractional change for diagnostics
    delta_rs_over_rs = (r_s_mtdf - r_s_std) / r_s_std

    # === CMB OBSERVABLES ===
    # R: Shift parameter (uses standard Omega_m, NOT affected by EFE)
    R = np.sqrt(Omega_m) * D_M_star / D_H

    # lA: Acoustic scale with BASELINE r_s (for comparison)
    lA_std = np.pi * D_M_star / r_s_std

    # lA: Acoustic scale with CORRECTED r_s (the EFE prediction)
    lA_mtdf = np.pi * D_M_star / r_s_mtdf

    # Angular sound horizon (theta_s = r_s / D_A, where D_A = D_M / (1+z))
    theta_s_std = r_s_std / (D_M_star / (1 + z_star)) * 180.0 / np.pi * 60.0  # arcmin
    theta_s_mtdf = r_s_mtdf / (D_M_star / (1 + z_star)) * 180.0 / np.pi * 60.0  # arcmin

    # Build prediction vector [R, lA_mtdf, omegab_h2]
    prediction = np.array([R, lA_mtdf, omegab_h2])
    ...
    if return_diagnostics:
        diagnostics = {
            ...
            'lA_mtdf': lA_mtdf,
            'theta_s_mtdf': theta_s_mtdf,

            # Final observables
            'R': R,
            'lA': lA_mtdf,  # The EFE-corrected value
            'omegab_h2': omegab_h2,
